...
private:
	...
	bool isCapFeasible;
	bool isPathLengthFeasible;
public:
	Chromosome();
	Chromosome(const Chromosome & chrom);
	Chromosome(std::vector<int> chromosome,	double fitness,	int sepInt,	bool isCapFeasible, bool isPathLengthFeasible, int maxRouteCapacity, double maxPathLength);
...
class Population {
	...
	double TSRunningTime = 0;
public:
	Population();
	Population(const Population & poplatn);
	Population(int populationSize,	int numberOfNodes,	int depotNode,	int capacityLimit, double maxPathLength,	int kChainLength,	int sWapLength, std::map<int, int> demand, std::vector<std::vector<double>> distance, std::vector<int> customerCluster);
...
private:
	...
	double GARunningTime = 0;
	double cumulativeTSRunningTime = 0;
public:
	Geneticalgorithm();
	Geneticalgorithm(const Geneticalgorithm & ga);	
	Geneticalgorithm(int populationSize, int numberOfNodes, int depotNode, int capacityLimit, double maxPathLength, int kChainLength, int sWapLength, std::map<int, int> demand, std::vector<std::vector<double>> distance, std::vector<int> customerCluster);
	Geneticalgorithm(int depotNode, int capacityLimit, double maxPathLength, std::map<int, int> demand, std::vector<std::vector<double>> distance, std::vector<int> customerCluster);
	...
	double returnGATime();
	double returnCumulativeTSTime();
	std::list<Chromosome> getLastPopulation();
};
