...
//constructor
FeasibleSolution::FeasibleSolution(std::list<int> sol, double cost, int sepIntVal, int srcRoute, int addNode, double maxPathLen){
	solCost			 = cost;
...
FeasibleSolution::FeasibleSolution(const FeasibleSolution & febSol) {
	...
	maxPathLength   = febSol.maxPathLength;
...
//constructor
Tabusearch::Tabusearch(FeasibleSolution febSol, std::map<int, int> demandVec, std::vector<std::vector<double>> disMat, int k_Chain, int swap_Chain, int maxCapacity, double maxPathLen) {
	kChain				        	= k_Chain;
...
//constructor
Tabusearch::Tabusearch(FeasibleSolution febSol, std::map<int, int> demandVec, std::vector<std::vector<double>> disMat, int maxCapacity, double maxPathLen) {
	kChain							= 5;
	swapChain						= 5;
	dropFromRoute					= 0;
	addToRoute						= 0;
...
Tabusearch::Tabusearch(const Tabusearch & tabusrch) {
	...
	maxRouteCapacity				= tabusrch.maxRouteCapacity;
	maxPathLength					= tabusrch.maxPathLength;
...
FeasibleSolution Tabusearch::generateNeighbourByAddDrop(std::list<std::list<int>> newRoutes, double newCost, int sepInt, int addToRoute, int dropFromRoute, int dropNode) {
	...
	for (auto &iter : newRoutes) {
		if (iterator == addToRoute) {
			...
			for (auto it = iter.begin(); it != iter.end(); ++it) {
				...
				if (cost < newAddRouteCost && cost <= maxPathLength) {
	...
	//incase node is not added
	if (nodeAdded == false) {
		std::list<int> newlist;
		newlist.push_back(dropNode);
		newRoutes.push_back(newlist);
	}
	...
	FeasibleSolution neighbour(solution, newCost, sepInt, dropFromRoute, dropNode, maxPathLength);
	return neighbour;
}
...
FeasibleSolution Tabusearch::generateNeighbourByOneSwap(std::list<std::list<int>> newRoutes, double newCost, int sepInt, int firstRoute, int secondRoute) {
	...
	FeasibleSolution neighbour(solution, newCost, sepInt, firstRoute, swapInfo.firstNode, maxPathLength);
	return neighbour;
}
...
void Tabusearch::runTabuSearch() {
	...
	//std::cout << "\nBefore running tsp heuristics." << std::endl;
    performTspHeuristics();
...
void Tabusearch::performTspHeuristics() {
	...
	/*
	for (auto &rt: incumbentRoutes) {
		for (auto it: rt) {
			std::cout << it << " ";
		}
		std::cout << "\nNew route." << std::endl;
	}
	std::cout << "\nEnd of the solution." << std::endl;
	*/
	//optimize the routes
	...
	for (auto& route : incumbentRoutes) {
		//run tsp heuristic
		int siz = route.size();
		//std::cout << "\nSize of the route : " << siz << std::endl;
		int MaxIter = 0;
		if (siz <= 4) {//later this number needs to be customized
			...
			//std::cout << "\nStart of the branch and bound alg." << std::endl;
			BranchAndBoundSolver bbSolver = BranchAndBoundSolver(tour, distanceMatrix);
			bbSolver.runBranchAndBoundSolver();
			//std::cout << "\nEnd of the branch and bound alg." << std::endl;
			TSPSolution tspSol = bbSolver.getTSPSolution();
			//check whether the tspSol is right
			std::set<int> setOfNodes;
			for (auto it: tspSol.tour) {
				setOfNodes.insert(it);
				//std::cout << "\nInsert node : " << it << std::endl;
			}
			bool isTspSolCorrect = true;
			for (auto it: tour) {
				auto itr = setOfNodes.find(it);
				if (itr == setOfNodes.end()) {
					isTspSolCorrect = false;
				}
			}
			//std::cout << "\nIs the solution correct? 0 for false and 1 for true." <<isTspSolCorrect<< std::endl;
			if (isTspSolCorrect == true) {
				//need to fix route for the sepIntVal node
				...
				//std::cout << "\nThe original route should not be changed as the current solution is wrong." << std::endl;
				MaxIter = 8;
				//start
				for (int iter = 1; iter <= MaxIter; iter++) {
					double costA = 0;
	...
	*/
	...
	//std::cout << "\nStart febsol classs." << std::endl;
	FeasibleSolution febSol(updatedSol, totalCost, sepIntVal, sourRt, addedNod, maxPathLength);
	//std::cout << "\nEnd of making feasible solution." << std::endl;
	incumbentSolution = febSol;
