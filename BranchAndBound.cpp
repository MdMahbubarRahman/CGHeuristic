...
BranchAndBoundSolver::BranchAndBoundSolver(const BranchAndBoundSolver& depthFirstBBSolver) {
	...
	isASolCorrect = depthFirstBBSolver.isASolCorrect;
}
...
void BranchAndBoundSolver::solveAssignmentProblem() {
	...
	isASolCorrect = hungAlg.isAssignmentSolCorrect();
	if (isASolCorrect == true) {
		bool initTSPOptimal = hungAlg.getInitialTSPStatus();
		if (initTSPOptimal == false) {
...
void BranchAndBoundSolver::pruneNodeByIntegrality() {
	...
	//std::cout << "\nEnd of the prune by integralilty function." << std::endl;
}
...
void BranchAndBoundSolver::solveNodeByCostOperator(Node node) {
	...
	//std::cout << "\nBefore running cost operator." << std::endl;
	opThry.runCostOperatorForSolvingANode();
	//std::cout << "\nAfter running cost operator." << std::endl;
	...
	//opThry.showChildNodes();
	//std::cout << "\nEnd of the solvenodebycostoperator function." << std::endl;
}
...
void BranchAndBoundSolver::runBranchAndBoundSolver() {
	...
	if (isASolCorrect == true) {
		//std::cout << "\nBBTree has been initiated." << std::endl;
		//std::cout << "\n bstatus : " << bStatus << std::endl;
		if (bStatus != Optimal) {
			while (!bbTree.branchNodes.empty()) {
				...
				if (bStatus == Optimal) {
					...
					//std::cout << "\nONe node has been selected." << std::endl;
			...
			for (auto& it : incumbent.tour) {
				...
				//std::cout << "\nOld city id : " << val << std::endl;
			}
		}
		else {
			tspSolution.tour.push_back(0);
		}
	}
	else {
		tspSolution.tour.push_back(0);
