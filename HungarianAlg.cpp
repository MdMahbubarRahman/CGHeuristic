...
HungarianAlg::HungarianAlg(const HungarianAlg& hunAlg) {
	...
	isAssignSolCorrect = hunAlg.isAssignSolCorrect;
}
...
bool HungarianAlg::getInitialTSPStatus() {
	return tspOptimal;
}

//check correctness of the assignment solution.
bool HungarianAlg::isAssignmentSolCorrect() {
	bool feasible = true;
	std::map<int, int> assSol = boxPoints;
	std::vector<int> firstVec;
	std::vector<int> secondVec;
	for (auto it: assSol) {
		firstVec.push_back(it.first);
		secondVec.push_back(it.second);
	}
	//firstVec
	for (auto it : firstVec) {
		int i = 0;
		for (auto itt : firstVec) {
			if (it == itt) {
				i += 1;
				if (i >= 2) {
					feasible = false;
	...
	isAssignSolCorrect = feasible;
	return isAssignSolCorrect;
}